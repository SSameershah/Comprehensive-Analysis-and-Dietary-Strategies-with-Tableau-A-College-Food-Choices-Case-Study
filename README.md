...
📄 Full Report
See report/College_Food_Choices_Case_Study.pdf for the complete write-up: executive summary, dataset description, step-by-step Tableau methodology, 8 visual analyses, and 6 actionable dietary strategies.
...
Structured after the well-known Kaggle "Food Choices" college student survey. Swap this file for a real Kaggle export with matching column names to reproduce the analysis on genuine data — see scripts/generate_data.py for the exact schema.
...
Open Tableau Desktop/Public → Connect → Text File → select data/college_food_choices.csv.
Set class_year, eating_out_frequency, etc. as Dimensions; gpa, bmi_est as Measures.
Build one worksheet per question (see Section 3 of the PDF report for the full list).
Add filter/highlight dashboard actions to cross-filter all charts.
Combine into a Dashboard, then a Story with 3-4 narrative points.
